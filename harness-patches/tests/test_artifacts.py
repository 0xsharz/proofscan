...
from harness.artifacts import FindingArtifact, GraderVerdict, RunResult


def test_finding_artifact_roundtrip():
    orig = FindingArtifact(
        ...
        finding_evidence="==12345==ERROR: AddressSanitizer: heap-buffer-overflow\n  #0 0xdeadbeef",
        exit_code=134,
        dup_check="Checked: top frame decode_chunk via parse_foo — no match in Already Filed.",
    )
    restored = FindingArtifact.from_dict(orig.to_dict())
    assert restored == orig
    assert restored.poc_bytes == orig.poc_bytes  # bytes survived b64 round-trip


def test_finding_artifact_from_dict_missing_dup_check():
    # Older result.json files won't have this key
    d = FindingArtifact(
        poc_path="/tmp/x", poc_bytes=b"\x00", reproduction_command="/work/entry /tmp/x",
        finding_type="SEGV", finding_evidence="trace", exit_code=134,
    ).to_dict()
    del d["dup_check"]
    assert FindingArtifact.from_dict(d).dup_check is None
...
def test_grader_verdict_roundtrip():
    orig = GraderVerdict(
        ...
        evidence="PoC is 847 bytes, 3/3 runs reproduced.",
    )
...
def test_run_result_json_roundtrip():
    ...
    assert restored.finding == orig.finding
...
def test_run_result_no_finding():
    orig = RunResult(
        target="stb",
        status="no_finding",
        finding=None,
    ...
    restored = RunResult.from_json(orig.to_json())
    assert restored.finding is None
    assert restored.verdict is None
    assert restored.status == "no_finding"
