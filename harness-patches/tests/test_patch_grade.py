...
import pytest
from harness.artifacts import FindingArtifact, PatchVerdict
...
def test_t0_fail_short_circuits(mock_docker):
    ...
    v = asyncio.run(
        grade_patch(CANARY, ALPHA_FINDING, DIFF, model="m", run_reattack=False)
    )
...
def test_multi_diff_reland_collapses(mock_docker):
    ...
    v = asyncio.run(
        grade_patch(CANARY, ALPHA_FINDING, [DIFF, DIFF], model="m", run_reattack=False)
    )
...
def test_t2_runs_when_configured(mock_docker):
    ...
    v = asyncio.run(
        grade_patch(target, ALPHA_FINDING, DIFF, model="m", run_reattack=False)
    )
...
def test_reattack_clean_when_no_crash(mock_docker):
    ...
    with patch(
        ...
        v = asyncio.run(grade_patch(CANARY, ALPHA_FINDING, DIFF, model="m"))
...
def test_reattack_dirty_when_same_signature(mock_docker):
    ...
    same_finding = FindingArtifact(
        poc_path="/tmp/p",
        poc_bytes=b"x",
        reproduction_command="/work/entry /tmp/p",
        finding_type="heap-buffer-overflow",
        finding_evidence="    #1 0x40 in parse_alpha /work/entry.c:26\n",
        exit_code=134,
    )
    with patch(
        "harness.patch_grade.run_find",
        new=AsyncMock(return_value=(same_finding, None, {})),
    ):
        v = asyncio.run(grade_patch(CANARY, ALPHA_FINDING, DIFF, model="m"))
...
def test_reattack_any_crash_fails(mock_docker):
    ...
    other_finding = FindingArtifact(
        poc_path="/tmp/p",
        poc_bytes=b"x",
        reproduction_command="/work/entry /tmp/p",
        finding_type="stack-buffer-overflow",
        finding_evidence="    #1 0x40 in parse_bravo /work/entry.c:38\n",
        exit_code=134,
    )
    with patch(
        "harness.patch_grade.run_find",
        new=AsyncMock(return_value=(other_finding, None, {})),
    ):
        v = asyncio.run(grade_patch(CANARY, ALPHA_FINDING, DIFF, model="m"))
...
def test_no_build_command_raises():
    ...
    with pytest.raises(ValueError, match="build_command"):
        asyncio.run(grade_patch(target, ALPHA_FINDING, DIFF, model="m"))
