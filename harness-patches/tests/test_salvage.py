...
from harness.artifacts import FindingArtifact, GraderVerdict
...
def _finding() -> FindingArtifact:
    return FindingArtifact(
        ...
        finding_evidence="SUMMARY: AddressSanitizer: heap-buffer-overflow on x",
        exit_code=1,
...
def _run_vp(tmp_path, monkeypatch, finding, find_error):
    target = TargetConfig.load(REPO / "targets" / "canary")
    find_result = AgentResult(messages=[_assistant("done")], error=find_error)
    graded: list[FindingArtifact] = []

    async def fake_run_find(*a, **k):
        return finding, find_result, {"find": 1.0}

    async def fake_run_grade(finding_arg, *a, **k):
        graded.append(finding_arg)
...
def test_vp_error_after_submission_salvages_finding(tmp_path, monkeypatch):
    result, on_disk, graded = _run_vp(
        tmp_path, monkeypatch, _finding(), "turn budget exhausted"
    )
    assert result.status == "finding_confirmed"
    assert len(graded) == 1  # the submitted finding reached the grader
...
def test_vp_clean_run_records_no_error(tmp_path, monkeypatch):
    result, _, graded = _run_vp(tmp_path, monkeypatch, _finding(), None)
    assert result.status == "finding_confirmed"
