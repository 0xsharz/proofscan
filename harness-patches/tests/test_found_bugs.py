...
from harness.artifacts import FindingArtifact
...
def test_stops_at_second_stack_section():
    ...
    # leak `malloc` or the alloc-site frame into the finding summary.
    frame = top_frame(ASAN_TRACE)
    assert frame is not None
...
# ── jsonl append/read round-trip (raw evidence excerpt format) ───────────────

def _mk_finding(finding_type="heap-buffer-overflow", finding_evidence=ASAN_TRACE):
...
def test_append_writes_raw_excerpt(tmp_path):
    ...
    _append_found(p, _mk_finding(), run_idx=3)

    entry = json.loads(p.read_text().strip())
    assert entry["run_idx"] == 3
    assert "evidence_excerpt" in entry
    assert "heap-buffer-overflow" in entry["evidence_excerpt"]
    assert "decode_chunk" in entry["evidence_excerpt"]
    # Old pre-parsed fields are gone — agents parse raw evidence themselves now.
    assert "finding_type" not in entry
...
def test_append_and_read_excerpts(tmp_path):
    p = tmp_path / "found_bugs.jsonl"
    _append_found(p, _mk_finding(), run_idx=0)
    _append_found(p, _mk_finding(finding_evidence=ASAN_TRACE_INTERCEPTOR), run_idx=1)
...
def test_seed_then_append_mixed_formats(tmp_path):
    # Config-seeded entries are prose summaries; runtime entries are evidence excerpts.
    ...
    assert "decode_chunk" in entries[1]  # evidence excerpt
...
def test_concurrent_section_mentions_evidence_excerpt():
    # The comparison guidance references raw evidence, not a pre-parsed top frame
    p = build_find_prompt("url", "abc", "/src", "/bin",
                          found_bugs_path="/r/found.jsonl")
    sect = p[p.index("## Concurrent Agents"):]
    assert "evidence" in sect.lower()
