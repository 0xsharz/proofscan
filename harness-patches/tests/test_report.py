...
from harness.novelty import finding_file_from_frame
...
# ── finding_file_from_frame ──────────────────────────────────────────────────

def test_finding_file_from_frame():
    assert finding_file_from_frame("stbi__out_gif_code /work/stb_image.h:6668") == "/work/stb_image.h"
    assert finding_file_from_frame("decode_chunk /src/a/b/decoder.c:4521") == "/src/a/b/decoder.c"
    assert finding_file_from_frame("memcpy") is None
    assert finding_file_from_frame("<no-frame>") is None
...
def _kwargs(**over):
    d = dict(
        ...
        finding_evidence="==1==ERROR: AddressSanitizer: heap-buffer-overflow\n",
        attack_surface=None, upstream_log=None, finding_file=None,
    )
    d.update(over)
    return d
...
def test_build_prompt_has_raw_asan_no_preparse():
    # Report agent reads raw evidence, not pipeline-preparsed finding_type/top_frame
    p = build_report_prompt(**_kwargs())
    assert "Static severity" not in p
    assert "Top frame" not in p
    assert "classify the finding type" in p.lower()
...
def _mk_result(tmp_path, name, status, score, poc_len):
    ...
    r = {
        ...
        "finding": {
            ...
            "reproduction_command": "/work/entry /tmp/poc.bin",
            "finding_type": "heap-buffer-overflow",
            "finding_evidence": "SUMMARY: AddressSanitizer: heap-buffer-overflow\n",
            "exit_code": 134, "dup_check": "novel",
        },
        "verdict": {"passed": status == "finding_confirmed", "score": score,
...
def test_representative_prefers_passed(tmp_path):
    a = _mk_result(tmp_path, "a", "finding_rejected", 0.6, 10)
    b = _mk_result(tmp_path, "b", "finding_confirmed", 0.2, 10)
    entries = [(a, "finding_rejected", {}), (b, "finding_confirmed", {})]
    path, _r, _c = _pick_representative(entries)
    assert path == b


def test_representative_prefers_higher_score(tmp_path):
    a = _mk_result(tmp_path, "a", "finding_confirmed", 0.8, 10)
    b = _mk_result(tmp_path, "b", "finding_confirmed", 1.0, 100)
    entries = [(a, "finding_confirmed", {}), (b, "finding_confirmed", {})]
    path, _r, _c = _pick_representative(entries)
    assert path == b


def test_representative_prefers_smaller_poc(tmp_path):
    a = _mk_result(tmp_path, "a", "finding_confirmed", 1.0, 500)
    b = _mk_result(tmp_path, "b", "finding_confirmed", 1.0, 35)
    entries = [(a, "finding_confirmed", {}), (b, "finding_confirmed", {})]
    path, _r, _c = _pick_representative(entries)
    assert path == b
