...
def test_judge_prompt_with_manifest():
    entries = [
        {"bug_id": 0, "run_idx": 3, "evidence_excerpt": "SEGV in out_gif_code /work/img.h:6668",
         "report_text": None},
        {"bug_id": 1, "run_idx": 7, "evidence_excerpt": "stack-buffer-overflow in parse_bravo",
         "report_text": "<primitive>Confirmed WRITE of 17 bytes past buffer.</primitive>"},
    ]
    p = build_judge_prompt(
        evidence_excerpt=ASAN_EXCERPT, dup_check="novel",
        grade_status="finding_confirmed", grade_score=1.0, poc_size=47,
...
def test_manifest_append_and_read(tmp_path):
    ...
    assert "SEGV" in entries[0]["evidence_excerpt"]
