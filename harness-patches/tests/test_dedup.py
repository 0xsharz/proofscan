...
"""Post-hoc finding deduplication: signature extraction + results-tree grouping.

Summary-only view now — the judge agent gates report dispatch in streaming
mode; dedup just answers "these N findings cluster into M signatures".
"""
import json
...
def test_signature_basic():
    finding = {"finding_type": "heap-buffer-overflow", "finding_evidence": ASAN_TRACE_A}
    assert _signature(finding) == ("heap-buffer-overflow", "decode_chunk /work/decoder.h:4521")


def test_signature_skips_interceptor():
    finding = {"finding_type": "stack-buffer-overflow", "finding_evidence": ASAN_TRACE_B}
    assert _signature(finding) == ("stack-buffer-overflow", "parse_bravo /work/entry.c:40")


def test_signature_assertion():
    # glibc assert() has no #N frames; same function as ASAN_TRACE_A, same dedup shape
    finding = {"finding_type": "assertion-failure", "finding_evidence": ASSERTION_OUTPUT}
    assert _signature(finding) == ("assertion-failure", "decode_chunk /work/decoder.h:4521")


def test_signature_no_frame_fallback():
    finding = {"finding_type": "SEGV", "finding_evidence": "<no parseable stack>"}
    assert _signature(finding) == ("SEGV", NO_FRAME)


def test_signature_missing_fields():
    assert _signature({}) == ("unknown", NO_FRAME)
    assert _signature({"finding_type": None, "finding_evidence": None}) == ("unknown", NO_FRAME)


# ── dedup (results tree walk) ────────────────────────────────────────────────

def _write_result(path, status, finding_type=None, finding_evidence=None):
    ...
    path.write_text(json.dumps({
        "target": "synthetic", "status": status, "finding": finding,
...
def _build_tree(tmp_path):
    """3 findings on signature A, 1 on B, plus a no-finding and a rejected-on-A."""
    root = tmp_path / "results" / "synthetic" / "20260101T000000Z"

    # run_000, run_002: same bug, both passed
    _write_result(root / "run_000" / "result.json", "finding_confirmed",
                  "heap-buffer-overflow", ASAN_TRACE_A)
    _write_result(root / "run_002" / "result.json", "finding_confirmed",
                  "heap-buffer-overflow", ASAN_TRACE_A)

    # run_003: same bug, grader bounced it — still groups with the above
    _write_result(root / "run_003" / "result.json", "finding_rejected",
                  "heap-buffer-overflow", ASAN_TRACE_A)

    # run_001: different bug
    _write_result(root / "run_001" / "result.json", "finding_confirmed",
                  "stack-buffer-overflow", ASAN_TRACE_B)

    # run_004: no finding — should be skipped
    _write_result(root / "run_004" / "result.json", "no_finding")
...
def test_dedup_includes_rejected(tmp_path):
    ...
    assert statuses == {"finding_confirmed", "finding_rejected"}
...
def test_dedup_skips_null_finding(tmp_path):
    root = _build_tree(tmp_path)
    groups = dedup(root)

    # 5 result.json files, but only 4 have a finding
...
def test_dedup_walks_nested_batches(tmp_path):
    ...
    _write_result(target_root / "20260101T000000Z" / "result.json", "finding_confirmed",
                  "heap-buffer-overflow", ASAN_TRACE_A)
    _write_result(target_root / "20260102T000000Z" / "result.json", "finding_confirmed",
                  "heap-buffer-overflow", ASAN_TRACE_A)
...
def test_format_report_shows_status(tmp_path):
    root = _build_tree(tmp_path)
    report = format_report(dedup(root), root)
    assert "(finding_confirmed)" in report
    assert "(finding_rejected)" in report


def test_format_report_empty():
    assert format_report({}) == "No findings found.\n"
