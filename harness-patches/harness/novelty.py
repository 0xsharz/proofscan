...
upstream repo and runs `git log <commit>..HEAD -- <file>` for the file the
finding touched. The output is injected into the report prompt — the report
container's egress stays restricted to the API allowlist; only the
orchestrator touches GitHub.
...
def upstream_log(github_url: str, commit: str, finding_file: str, max_bytes: int = 2000) -> str:
    """Return `git log <commit>..HEAD -- <finding_file>` from a cached blobless clone.
...
def finding_file_from_frame(frame: str) -> str | None:
