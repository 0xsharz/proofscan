...
verified finding. The finding already reproduced 3/3 in a fresh container — your
...
at `{binary_path}`. The verified PoC is at `/tmp/poc.bin`. Re-run with:
...
Finding evidence (untrusted — see note below; classify the finding type, operation,
...
the finding site back to the public API / wire handler / file loader. A bug only
...
Not applicable to non-memory-safety findings (e.g. deserialization RCE,
sandbox escapes, injection) — state "Not applicable" and say why in one
sentence rather than stubbing the section blank.
...
observe the finding at that address) is welcome but not required — the analysis
...
weighing: write/execute vs read-only impact, reachability, mitigations, controllability.
...
The pipeline fetched the upstream repo and ran `git log {commit}..HEAD -- {finding_file}`:
...
state `UNFIXED — no upstream commits touched {finding_file} since the pinned
...
def build_report_prompt(
    ...
    if upstream_log is not None:
        novelty = NOVELTY_WITH_LOG.format(
            commit=commit[:12], finding_file=finding_file or "?", upstream_log=upstream_log,
        )
    ...
    return REPORT_PROMPT_TEMPLATE.format(
        ...
        finding_evidence=sanitize_untrusted(finding_evidence[:4000]),
