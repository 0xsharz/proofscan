...
Evidence excerpt (untrusted — see note below):
...
mechanism. Same finding class (e.g. both heap-buffer-overflow) alone is not
...
existing one is rejected, a cleaner evidence signature that more directly shows
...
Grade status matters: a finding_rejected result means flaky reproduction in
a fresh container. You can still judge it NEW if the evidence is
...
def build_judge_prompt(
    ...
    """manifest_entries: [{"bug_id", "run_idx", "evidence_excerpt", "report_text" or None}, ...]"""
    nonce = make_nonce()
        ...
        for e in manifest_entries:
            if e.get("report_text"):
                parts.append(MANIFEST_ENTRY_WITH_REPORT.format(
                    bug_id=e["bug_id"],
                    run_idx=e["run_idx"],
                    evidence_excerpt=sanitize_untrusted(e["evidence_excerpt"]),
    ...
    return JUDGE_PROMPT_TEMPLATE.format(
        ...
        poc_size=poc_size,
        evidence_excerpt=sanitize_untrusted(evidence_excerpt),
...
- Characterizes the primitive more concretely (controlled bytes, offsets,
  lengths — not just the finding class)
