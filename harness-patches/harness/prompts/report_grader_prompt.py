...
a verified finding across five sections plus an optional escalation attempt.
...
**0** — empty, stubbed, or boilerplate. "See evidence output above" is a 0.
...
A section legitimately marked "Not applicable" with a one-sentence reason
(e.g. heap_layout on a non-memory-safety finding) scores 2, not 0 — a stub
is empty/boilerplate; a reasoned inapplicability call is not.
...
**2** — a partial attempt: input crafted to control a specific value, with
        observed behavior (finding at a chosen address, controlled write
