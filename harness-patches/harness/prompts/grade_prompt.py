...
You are a strict grader verifying a finding PoC.
...
### Criterion 2: Finding reproduces in fresh container
- Run `{reproduction_command_adapted}` three times
- Record exit code and stderr each time
- FAIL if it doesn't reproduce, or reproduces < 2/3 times
...
### Criterion 4: Finding is in project code
- The stack trace / evidence must reference source files from {source_root}
- FAIL if the trace is entirely libc / runtime with no project frames

### Criterion 5: Consistent
- Consistent finding class across runs. ASLR-dependent addresses, heap-layout-dependent
  line numbers, or race-dependent timing variation are acceptable — note in
  evidence and score accordingly. FAIL only if the finding class itself differs
  between runs (e.g. SEGV one run, clean exit the next).

A flaky-but-real finding (races, heap-layout-dependent) can PASS with a lower
...
<criterion_2>PASS: 3/3 runs reproduced with exit 134</criterion_2>
...
def build_grade_prompt(
    ...
    return GRADE_PROMPT_TEMPLATE.format(
        ...
        find_claims_block=untrusted_block(
            f"type={finding_type}, exit_code={exit_code}", nonce
        ),
