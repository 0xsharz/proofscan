...
"""CLI entrypoint.
  ...
  vuln-pipeline dedup <results_dir>                            # group findings by signature
  vuln-pipeline report <results_dir> --model <m> [--novelty]   # exploitability analysis per unique finding
...
from .artifacts import FindingArtifact, RunResult
from .asan import evidence_excerpt, finding_reason, top_frame
...
from .novelty import upstream_log, finding_file_from_frame, NOVELTY_NOT_CHECKED
...
_RUN_TERMINAL = {"finding_confirmed", "finding_rejected", "no_finding"}
...
def _write_result(out_dir: Path, result: RunResult) -> None:
    ...
    if result.finding:
        with open(out_dir / "poc.bin", "wb") as f:
            f.write(result.finding.poc_bytes)
    ...
    # Pipeline-parsed classification: deterministic finding_type / severity /
    # operation. Sits alongside the agent-emitted finding_type so downstream
    # consumers can cross-check (the agent tag is free-text and fragments).
    if result.finding:
        slim["finding"]["reason"] = finding_reason(result.finding.finding_evidence)
...
async def _run_once(
    ...
    for graded findings (passed or rejected) and appends any spawned report
    ...
    try:
        finding, find_result, find_timings = await run_find(
        ...
        return _done(RunResult(
            ...
            finding=None, verdict=None, timings=timings,
    ...
    if find_result.error:
        ...
        # bytes are already extracted from the container — grade the finding
        ...
        print(f"[find:{run_idx}] Agent error after submission "
              f"({find_result.error}) — salvaging the submitted finding.")

    if finding is None:
        print(f"[find:{run_idx}] No finding artifact emitted.")
        return _done(RunResult(
            target=target.name, status="no_finding",
            finding=None, verdict=None,
            find_transcript=find_transcript, timings=timings,
        ))

    print(color(f"[find:{run_idx}] Finding claimed: {finding.finding_type} at {finding.poc_path} ({len(finding.poc_bytes)} bytes)", "red"))
    ...
    # finding doesn't pollute siblings' dedup context.
    if finding.dup_check is None:
        print(f"[find:{run_idx}] Rejected: missing <dup_check> tag.")
        return _done(RunResult(
            target=target.name, status="agent_failed",
            finding=finding, verdict=None,
            find_transcript=find_transcript, timings=timings,
    ...
    # Entries are framed as "claims" in the prompt, not confirmed findings.
    if found_bugs_path:
        _append_found(found_bugs_path, finding, run_idx)

    if find_only:
        return _done(RunResult(
            target=target.name, status="no_finding",  # ungraded → not confirmed
            finding=finding, verdict=None,
            find_transcript=find_transcript, timings=timings,
        ))
    ...
    try:
        verdict, grade_result, grade_elapsed = await run_grade(
            finding, target, model=model, workspace_dir=str(workspace), agent_env=agent_env,
    ...
    status = "finding_confirmed" if verdict.passed else "finding_rejected"
    result = RunResult(
    ...
    if stream_ctx is not None:
        try:
            await _stream_dispatch(run_idx, target, model, agent_env, finding,
...
async def _stream_dispatch(
    ...
    excerpt = evidence_excerpt(finding.finding_evidence)
    ...
    async with ctx["lock"]:
        ...
        jv, _jr, elapsed = await run_judge(
            evidence_excerpt=excerpt, dup_check=finding.dup_check,
            grade_status=grade_status, grade_score=grade_score,
            poc_size=len(finding.poc_bytes),
    ...
    task = asyncio.create_task(_stream_report(
        run_idx, bug_id, finding, target, model, agent_env,
...
async def _stream_report(
    ...
    """Write an exploitability report for one finding. If re_report, preserve
    ...
    frame = top_frame(finding.finding_evidence) or ""
    finding_file = finding_file_from_frame(frame)
    log = None
    if novelty:
        print(f"[report:{run_idx}→bug_{bug_id:02d}] novelty: fetching upstream log for {finding_file or '?'} ...")
        log = upstream_log(target.github_url, target.commit,
                           finding_file or "", max_bytes=2000)

    print(color(f"[report:{run_idx}→bug_{bug_id:02d}] starting ({len(finding.poc_bytes)}B PoC) ...", "report"))
    try:
        verdict, report_text, result, elapsed = await run_report(
            finding, target, model=model,
            workspace_dir=str(out_dir / "workspace"),
            upstream_log=log, finding_file=finding_file,
            agent_env=agent_env,
    ...
    out = {
        "signature": {"finding_type": finding.finding_type, "top_frame": frame},
...
# ── found_bugs.jsonl: runtime finding-sharing ───────────────────────────────────
...
def _append_found(path: Path, finding: FindingArtifact, run_idx: int) -> None:
    # Raw evidence excerpt — SUMMARY line + first stack frames. Agents parse the
    # signature themselves; the pipeline doesn't pre-canonicalize finding_type or
    # top_frame anymore (that was a fragility point — adjacent lines, format
    # variance, free-text agent tags all fragmented the dedup).
    entry = {
        "run_idx": run_idx,
        "evidence_excerpt": evidence_excerpt(finding.finding_evidence),
    }
...
def _read_found_summaries(path: Path) -> list[str]:
    ...
    for line in path.read_text().splitlines():
        ...
        try:
            ...
            continue
        # Config-seeded entries are prose; runtime entries carry evidence excerpts.
        out.append(d.get("evidence_excerpt") or d.get("summary") or "")
...
def _append_manifest(reports_root: Path, bug_id: int, run_idx: int,
    ...
    with open(reports_root / "manifest.jsonl", "a") as f:
        f.write(json.dumps({
            "bug_id": bug_id, "run_idx": run_idx, "evidence_excerpt": excerpt,
        }) + "\n")
...
async def _run_all(
    ...
    # Shared file for runtime finding-sharing. Only wire it up for multi-run — a
    ...
    async def _checkpointed(i: int) -> RunResult:
        r = checkpoints[i]
        # Replay graded findings through judge→report so a kill between
        # _write_result and _stream_dispatch doesn't strand them. judge_log
        # is the per-run idempotence key.
        if (stream_ctx is not None and r.finding is not None
                and r.verdict is not None and i not in judged):
            try:
                await _stream_dispatch(i, target, args.model, agent_env, r.finding,
...
# (no confirmed finding, nothing to dedup, report/patch not submitted/verified).
def main() -> int:
    ...
    p_run.add_argument("--accept-dos", dest="accept_dos", action="store_true",
                       help="Benchmark mode — DoS-class findings (allocation-size-too-big, "
    ...
    p_dedup = sub.add_parser("dedup", help="Group findings under a results dir by signature")
    p_dedup.add_argument("results_dir", type=Path,
                         help="Directory to walk for result.json files (e.g. results/<target>/)")

    p_report = sub.add_parser("report",
                              help="Generate exploitability reports for unique findings under a results dir")
    ...
    p_report.add_argument("--only-passed", action="store_true",
                          help="Skip groups where no run passed grading (default: include finding_rejected)")
    ...
    p_patch = sub.add_parser("patch",
                             help="Generate and verify a fix for each unique finding under a results dir")
...
def _cmd_run(args) -> int:
    ...
    for i, (out_dir, result) in enumerate(pairs):
        ...
        print(color(_sline, "red") if result.status == "finding_confirmed" else _sline)
        if result.status != "finding_confirmed":
            exit_code = 2
    if args.stream:
...
_STATUS_ORDER = {"finding_confirmed": 0, "finding_rejected": 1}
...
def _pick_representative(entries: list[tuple[Path, str, dict]]) -> tuple[Path, dict, dict]:
    ...
    (cleaner to analyze). Returns (result_path, result_dict, finding_dict).
    ...
    for path, status, _reason in entries:
        ...
        poc_len = len(finding.get("poc_bytes") or "")
        key = (_STATUS_ORDER.get(status, 2), -score, poc_len, str(path))
        candidates.append((key, path, r, finding))

    if not candidates:
        raise ValueError("no readable result.json in group")
    _k, path, result, finding = min(candidates, key=lambda c: c[0])
    return path, result, finding
...
async def _report_one(
    ...
    finding_type, frame = sig
    out_dir = reports_root / f"bug_{idx:02d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    rep_path, _result, finding_dict = _pick_representative(entries)
    finding = FindingArtifact.from_dict(finding_dict)

    finding_file = finding_file_from_frame(frame)
    log = None
    if args.novelty:
        print(f"[report:{idx}] novelty: fetching upstream log for {finding_file or '?'} ...")
        log = upstream_log(target.github_url, target.commit,
                           finding_file or "", max_bytes=2000)

    print(color(f"[report:{idx}] {finding_type} in {frame} "
                f"(from {rep_path.parent.name}, {len(finding.poc_bytes)}B PoC) ...", "report"))

    try:
        verdict, report_text, result, elapsed = await run_report(
            finding, target, model=args.model,
            workspace_dir=str(out_dir / "workspace"),
            upstream_log=log, finding_file=finding_file,
            agent_env=agent_env,
        ...
        traceback.print_exc()
        out = {"signature": {"finding_type": finding_type, "top_frame": frame},
    ...
    out = {
        "signature": {"finding_type": finding_type, "top_frame": frame},
...
def _load_report_checkpoint(out_dir: Path, sig: tuple[str, str]) -> dict | None:
    ...
    mismatch means bug_NN index drifted (e.g. --resume added new findings
    ...
    if d.get("status") != "report_submitted":
        return None
    s = d.get("signature", {})
    if (s.get("finding_type"), s.get("top_frame")) != sig:
        return None
    return d
...
def _cmd_report(args) -> int:
    ...
    groups = dedup(root)
    if not groups:
        print("No findings under results dir.", file=sys.stderr)
        return 2

    # Filter + order: passed groups first, then rejected (or drop if --only-passed).
    def _has_passed(entries): return any(s == "finding_confirmed" for _p, s, _r in entries)
    ...
    if not items:
        print("No passed-grade findings (use without --only-passed to include rejected).",
              file=sys.stderr)
        return 2
...
def _cmd_patch(args) -> int:
    ...
    def _has_passed(ents): return any(s == "finding_confirmed" for _p, s, _r in ents)
    ...
    async def _one(idx: int, entries) -> dict:
        ...
        try:
            diff, verdict, _ = await run_patch(
                finding, target, model=args.model, out_dir=out_dir,
