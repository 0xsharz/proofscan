...
from .artifacts import FindingArtifact, GraderVerdict
...
async def run_grade(
    ...
    """Verify a FindingArtifact in a fresh container.
    ...
    if finding.poc_path not in finding.reproduction_command:
        raise ValueError(
            f"poc_path {finding.poc_path!r} not found in reproduction_command "
            f"{finding.reproduction_command!r} — find-agent output is inconsistent"
        )
