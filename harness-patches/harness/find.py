...
from .artifacts import FindingArtifact
...
async def run_find(
...
) -> tuple[FindingArtifact | None, AgentResult, dict[str, float]]:
    """Run one find attempt against a target.

    Returns (finding_or_none, agent_result, timings).
    finding is None if no PoC was emitted or the claimed path was empty.
    ...
    with sandbox.agent_container(
        ...
        finding_type = parse_xml_tag(text, "finding_type")
        finding_evidence = parse_xml_tag(text, "finding_evidence") or ""
        ...
        finding = FindingArtifact(
            ...
            finding_evidence=finding_evidence[:10_000],  # traces can be huge; top is what matters
            exit_code=_parse_exit_code(exit_code_str),
            dup_check=dup_check,
        )
        return finding, result, timings
