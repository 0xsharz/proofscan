...
KNOWN LIMITATION: T1 (_t1_passes) and the DoS-safety option below check for the
literal string "AddressSanitizer:" — this ladder is currently only load-bearing
for native ASAN-instrumented targets (drlibs, cjson, ...). A non-memory-safety
target (PyYAML/ReportLab-style, using harness_oracle.py's SecurityOracle
banner) would need T1 extended to also treat "SecurityOracle:" in the output as
a still-failing PoC, and its own oracle-specific re-attack framing. Not fixed
here — `patch`/`--style` has not yet been run against a SecurityOracle-based
target; flag this if/when it is.
"""
...
from .artifacts import FindingArtifact, PatchVerdict
...
async def grade_patch(
    ...
    """Grade a candidate patch against a verified finding.
    ...
    if finding.poc_path not in finding.reproduction_command:
        raise ValueError(
            f"poc_path {finding.poc_path!r} not in reproduction_command "
            f"{finding.reproduction_command!r}"
        )
    ...
    try:
        ...
        with sandbox.agent_container(
            ...
            s = time.time()
            await asyncio.to_thread(
                docker_ops.write_file, container, "/tmp/poc.bin", finding.poc_bytes
            )
            adapted = finding.reproduction_command.replace(finding.poc_path, "/tmp/poc.bin")
            try:
                rc, out, err = await asyncio.to_thread(
            ...
            if not run_reattack:
                ...
                focus = reattack_focus or _focus_hint(
                    finding, target.source_root if reattack_with_diff else None
                )
                re_finding, _, _ = await run_find(
                    patched_target,
                    model=model,
                ...
                if re_finding is None:
                    re_clean = True
                else:
                    re_clean = False
                    evidence["re_attack"] = _clip(re_finding.finding_evidence)
...
def _focus_hint(finding: FindingArtifact, diff_source_root: str | None = None) -> str:
    frames = project_frames(finding.finding_evidence, n=1)
    where = f" near {frames[0]}" if frames else ""
    hint = (
        f"Re-attack the just-patched code path. The original finding was "
        f"{finding.finding_type}{where}. Probe for a variant input that "
    ...
    if diff_source_root:
        hint += (
            ...
            f"report findings whose root cause is unrelated to the original."
        )
    return hint
