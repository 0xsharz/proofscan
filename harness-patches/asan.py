...
"""Crash-output parsing: native ASAN output, plus the SecurityOracle format
...
def crash_reason(crash_output: str) -> dict[str, str | None]:
    """crash_type + READ/WRITE operation parsed from sanitizer/oracle output.

    Display-only: feeds found_bugs.jsonl excerpts and dedup summary. Not a
    decision input — agents judge semantic duplicates from raw crash_output.
    """
    m = _ASAN_SUMMARY.search(crash_output) or _ORACLE_SUMMARY.search(crash_output)
